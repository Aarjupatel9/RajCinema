...
exports.getAllCanteens = async (req, res) => {
  try {
    const canteens = await canteenModel
      .find()
  ...
  try {
    if (canteenData._id == "") {
      ...
      res
        ...
        .json({
          success: true,
          message: "canteen successfully added",
          canteen: newProduct,
        });
    } else {
      const canteen = await canteenModel.findByIdAndUpdate(
        canteenData._id,
        canteenData,
        {
          new: true,
      ...
      res
        ...
        .json({
          success: true,
          message: "canteen successfully updated",
          canteen: canteen,
        });
    }
  } catch (error) {
...
exports.removeCanteen = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Canteen deleted successfully." });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal Server Error." });
  }
...
exports.getAllCanteenProducts = async (req, res) => {
  ...
  try {
    const product = await canteenModel
      .findById(canteenId)
