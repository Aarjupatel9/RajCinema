...
export default function Vendors() {
    ...
    useEffect(() => {
        ...
        const vendorPromise = userService.getAllVendors();
    ...
    function handleActualVendorRemove(id) {

        console.log("you said yes");
        // return;
        const vendorPromise = userService.removeVendorDetails(id);
        vendorPromise.then((res) => {
            const tmp = vendors.filter(vendor => vendor._id !== id);
            setVendors(tmp);
        })
        toast.promise(
            vendorPromise,
    ...
    function handelVendorDelete(id) {
        // console.log('handelVendorDelete start:');
        toast.custom((t) => (
            <div
                    ...
                    <div className='flex flex-row justify-start'>
                        <div className="flex md:order-2 mx-2">
                            <div className='bg-gray-300 dark:bg-gray-900'>
                                <Link to={"/addVendors"} className="block font-medium py-2 pl-3 pr-4 bg-gray-300 text-gray-950  rounded md:bg-transparent md:text-gray-950 text- md:p-0 md:dark:text-blue-500" aria-current="page">Add</Link>
                            </div>
                        </div>
                    ...
                    <div className="mt-4 relative overflow-x-auto shadow-md sm:rounded-lg">
                        <table className="w-full text-xl text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody className="">
                                {vendors.map((vendor) => {
                                    return (
                                        <tr key={vendor._id} className="border-b border-gray-200 dark:border-gray-600">
                                            ...
                                            <td className="px-6 py-4 text-right dark:text-white dark:bg-gray-700">
                                                <div onClick={() => { navigate("/addVendors/" + vendor._id); }} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</div>
                                            </td>
                                            <td className="px-6 py-4 text-right dark:text-white dark:bg-gray-700">
                                                <div onClick={() => { handelVendorDelete(vendor._id) }} className="font-medium text-red-600 dark:text-red-500 hover:underline">remove</div>
                                            </td>
                                        </tr>)
                                })
                                }
